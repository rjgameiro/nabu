...
# echo "$(date) - creating host entry: ${prefix}, ${entry}, ${file}, ${become_root}" >> output.actions
state=$(cat)
...
ansible-playbook \
  ...
  handlers/file-line-entry-create.yml > /dev/null 2>&1
[[ $? -ne 0 ]] && echo "Failed to create file entry." && exit 1
set -e
